...
class Aluno
{
    ...
    public function crud($opcao)
    {
        try {
            $this->con = new Conn();
            $sql = "CALL crud_aluno1(?, ?, ?, ?)";
            $executar = $this->con->prepare($sql);
            $executar->bindValue(1, $this->rm);
            $executar->bindValue(2, mb_strtoupper($this->idade));
    ...
    public function pesquisar($filtros)
    {
        ...
        $sql = "SELECT * FROM {$this->table} WHERE $where ORDER BY nome_aluno ASC";
